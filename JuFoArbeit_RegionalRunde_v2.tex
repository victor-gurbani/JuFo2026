...
\begin{description}
    \item[Humdrum-Ära (1990er):] Von Hippels Dissertation\footnote{\cite{VonHippel2000}.} etablierte statistische Melodieanalyse, fokussierte jedoch auf kognitive Erwartungsmodelle statt stilistischer Differenzierung.
    \item[MusicXML \& ML (2000er):] McKays jSymbolic-System\footnote{\cite{McKay2010}.} ermöglichte automatische Genre-Klassifikation mittels kNN/SVM, bot jedoch keine Interpretierbarkeit -- ein \enquote{Black-Box}-Ansatz.
    \item[music21-Revolution (2010er):] Whites Korpusanalysen\footnote{\cite{White2013}.} nutzten deskriptive Statistiken über große Zeiträume, ohne paarweise Komponistenvergleiche oder FDR-Korrektur.
    \item[Deep-Learning-Ära (2017--):] Hadjeres' DeepBach\footnote{\cite{Hadjeres2018}.} erreichte hohe Genauigkeit in Stil-Imitation, bleibt jedoch opak: \enquote{Warum klingt das wie Bach?} ist nicht beantwortbar.
\end{description}

\noindent Frühe Versuche wie Lin-Jengs Arbeit von 1987\footnote{\cite{LinJeng1987}.} litten unter kleinem Korpus, manuellem Encoding und begrenzten Werkzeugen. Aktuelle Übersichtsarbeiten kritisieren die Dominanz schwer interpretierbarer Modelle oder ungenügender Validierung.\footnote{\cite{Simonetta2025}.}

\textbf{Positionierung dieser Arbeit:} Das vorliegende Projekt kombiniert die Stärken aller Phasen: Skalierbarkeit (PDMX-Archiv mit $>$250\,000 Partituren\footnote{\cite{PDMX2024}.}), Interpretierbarkeit (36 explizite Merkmale statt verborgener Gewichte), statistische Rigorosität (ANOVA, Tukey-HSD, FDR-Korrektur) und Zugänglichkeit (\texttt{music21}\footnote{\cite{Cuthbert2010}.}, interaktive Visualisierungen). Tabelle~\ref{tab:evolution} vergleicht den methodischen Fortschritt.
...
\textbf{Chopins Brückenrolle:}\footnote{\cite{ChopinTransformations}. Chopin transformierte die Ausdrucksmittel, ohne das metrische Gerüst der Zeitvorgänger zu sprengen. Die Trennung der PCA-Cluster entlang unterschiedlicher Hauptkomponenten (PC1 für Harmonik vs. PC2/3 für Rhythmik) erklärt sich schlüssig aus dieser Dichotomie.} Die DD-Koeffizienten präzisieren Chopins katalytische Funktion mathematisch: harmonischer Innovator ($DD > 0$), rhythmischer Konservativer ($DD \approx 0$). Er dehnte tonale Grenzen bis zum Bruchpunkt, bewahrte aber rhythmische Symmetrie -- objektiv-mathematisch die Schnittstelle zwischen klassischer Ordnung und moderner Auflösung.
