...
Die computergestützte Stilanalyse hat seit den 1990er Jahren mehrere methodische Paradigmenwechsel erfahren. Jede Ära hinterließ spezifische Werkzeuge und Limitationen:

\begin{description}
    \item[Humdrum-Ära (1990er):] Von Hippels Dissertation\autocite{VonHippel2000} etablierte statistische Melodieanalyse, fokussierte jedoch auf kognitive Erwartungsmodelle statt stilistischer Differenzierung.
    \item[MusicXML \& ML (2000er):] McKays jSymbolic-System\autocite{McKay2010} ermöglichte automatische Genre-Klassifikation mittels kNN/SVM, bot jedoch keine Interpretierbarkeit -- ein \enquote{Black-Box}-Ansatz.
    \item[music21-Revolution (2010er):] Whites Korpusanalysen\autocite{White2013} nutzten deskriptive Statistiken über große Zeiträume, ohne paarweise Komponistenvergleiche oder FDR-Korrektur.
    \item[Deep-Learning-Ära (2017--):] Hadjeres' DeepBach\autocite{Hadjeres2018} erreichte hohe Genauigkeit in Stil-Imitation, bleibt jedoch opak: \enquote{Warum klingt das wie Bach?} ist nicht beantwortbar.
\end{description}

\noindent Frühe Versuche wie Lin-Jengs Arbeit von 1987\autocite{LinJeng1987} litten unter kleinem Korpus, manuellem Encoding und begrenzten Werkzeugen. Aktuelle Übersichtsarbeiten kritisieren die Dominanz schwer interpretierbarer Modelle oder ungenügender Validierung.\autocite{Simonetta2025}

\textbf{Positionierung dieser Arbeit:} Das vorliegende Projekt kombiniert die Stärken aller Phasen: Skalierbarkeit (PDMX-Archiv mit $>$250\,000 Partituren\autocite{PDMX2024}), Interpretierbarkeit (36 explizite Merkmale statt verborgener Gewichte), statistische Rigorosität (ANOVA, Tukey-HSD, FDR-Korrektur) und Zugänglichkeit (\texttt{music21}\autocite{Cuthbert2010}, interaktive Visualisierungen). \hyperref[tab:evolution]{Tabelle~\ref*{tab:evolution}} vergleicht den methodischen Fortschritt.

\begin{table}[H]
    \centering
    \caption{Evolution der Methodik im stilistischen Vergleich}\label{tab:evolution}
    \begin{tabular}{p{2.2cm}p{2.5cm}p{2.5cm}p{2.5cm}p{4.5cm}}
        \toprule
...
\hyperref[tab:features]{Tabelle~\ref*{tab:features}} erklärt zentrale Merkmale, die später als signifikant identifiziert wurden. Eine vollständige Dokumentation aller 36 Merkmale mit Berechnungsformeln und Beispielwerten findet sich in \hyperref[app:A]{Anhang~\ref*{app:A}}.
...
\begin{enumerate}
    ...
    \item \textbf{Multiple-Testing-Korrektur}: Bonferroni ist bei 36 Tests streng (14 statt 29 Treffer bei \(\alpha=0{,}05\)). Benjamini--Hochberg FDR (\(q<0{,}05\)) bestätigt in der aktuellen Auswertung ebenfalls 29 Merkmale; im Export werden dennoch FDR-adjustierte \textit{p}-Werte berichtet.\autocite[Vgl.][]{Benjamini1995}
...
Die gesamte Analysepipeline wurde als modulares CLI-System implementiert. Detaillierte Reproduzierbarkeitsanleitungen finden sich in \hyperref[app:B]{Anhang~\ref*{app:B}}. Ein vollständiger Durchlauf vom Rohdatensatz bis zu allen Abbildungen benötigt etwa 2--3 Stunden auf einem modernen Laptop.
...
\hyperref[fig:ambitus]{Abbildung~\ref*{fig:ambitus}} zeigt exemplarisch die Verteilungsunterschiede für den Tonumfang; weitere Boxplots für alle 36 Merkmale sind online verfügbar (siehe \hyperref[app:C]{Anhang~\ref*{app:C}}).
...
Die PCA-Visualisierung (\hyperref[fig:clouds]{Abbildung~\ref*{fig:clouds}}) fasst die 30 PCA-Eingangsmerkmale (36 Gesamtmerkmale abzüglich reiner Zähl-/Größenmerkmale) in drei Dimensionen zusammen. Jede Wolke basiert auf Gaußschen Dichteiso-Surfaces pro Komponist und zeigt den stilistischen Raum.
...
\noindent In der 5-Fold Cross-Validation erreichte dieser interpretierbare RF eine Genauigkeit von $\approx 70\,\%$. Die Verwechslungsmatrix (\hyperref[fig:rf_cm]{Abbildung~\ref*{fig:rf_cm}}) zeigt, dass Fehlklassifikationen meist zwischen chronologisch benachbarten Epochen (z.\,B. Chopin und Debussy) auftreten, was die These der fließenden Stilübergänge stützt.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{figures/random_forest/confusion_matrix.png}
    \caption{Verwechslungsmatrix des Random-Forest-Modells (5-Fold Cross-Validation). Die Diagonale zeigt die korrekten Zuweisungen. Die Diagonale zeigt die korrekten Zuweisungen. Auffällig ist, dass Werke von Chopin relativ häufig fälschlicherweise Bach (8-mal) oder Debussy (7-mal) zugeordnet werden, während Bach und Mozart sehr zuverlässig klassifiziert werden..}\label{fig:rf_cm}
\end{figure}

\subsubsection{Vergleich mit neuronalen Netzen (MLP)}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.75\textwidth]{figures/random_forest/classification_scatter.png}
    \caption{Precision-Recall-Scatterplot des Random Forests. Bach und Mozart weisen sowohl eine hohe Recall-Rate als auch Präzision auf und werden vom Modell am besten erkannt. Chopin hingegen fällt im Graphen deutlich ab, da das Modell Schwierigkeiten hat, seine stilistischen Muster eindeutig von den anderen Komponisten abzugrenzen.}\label{fig:rf_scatter}
\end{figure}

\subsection{Externer Falltest: Prädiktive Validierung}
...
\textbf{Fallstudie 1: Joe Hisaishi.} Die Filmmusik von Hayao Miyazakis Ghibli-Produktionen ist für ihre Synthese aus spätromantischer Harmonik und impressionistischen Klangfarben bekannt. Hisaishis \emph{One Summer's Day} (2001) wurde ohne Vorwissen in den PCA-Raum projiziert -- das Ergebnis (\hyperref[fig:hisaishi]{Abbildung~\ref*{fig:hisaishi}}) bestätigt die Hypothese: Das Stück positioniert sich präzise zwischen Chopin und Debussy, mit leichter Tendenz zu Chopin. Die Methode \emph{sagt korrekt voraus}, wo ein stilistisch hybrider Komponist landen sollte -- ein starker Beleg für die Validität des Merkmalsraums.
...
Die DD-Metrik (Abschnitt~5.3) erlaubt es, stilistischen Wandel als messbare \enquote{Veränderungsgeschwindigkeit} zu quantifizieren. Zentrale Befunde:

\textbf{Romantische Umkehr:} Der Übergang Bach$\to$Mozart zeigt strukturelle Destillation (Reduktion von Tonumfang und Dissonanz). Chopin markiert einen radikalen Trendbruch: Die massiven DD-Ausschläge (+25,14 Halbtöne; +0,31 Dissonanz) belegen, dass die Romantik keine graduelle Fortsetzung, sondern bewusste Ablehnung klassizistischer Restriktion war.

\textbf{Rhythmische Stagnation:} DD-Werte der rhythmischen Entropie verharren zwischen Klassik und Romantik nahe Null ($DD \approx 0$). Die rhythmische Befreiung kam erst mit Debussy (+1,43). Die romantische Revolution war primär harmonisch-melodisch.

\textbf{Chopins Brückenrolle:}\autocite[][Chopin transformierte die Ausdrucksmittel, ohne das metrische Gerüst der Zeitvorgänger zu sprengen. Die Trennung der PCA-Cluster entlang unterschiedlicher Hauptkomponenten (PC1 für Harmonik vs. PC2/3 für Rhythmik) erklärt sich schlüssig aus dieser Dichotomie.]{ChopinTransformations} Die DD-Koeffizienten präzisieren Chopins katalytische Funktion mathematisch: harmonischer Innovator ($DD > 0$), rhythmischer Konservativer ($DD \approx 0$). Er dehnte tonale Grenzen bis zum Bruchpunkt, bewahrte aber rhythmische Symmetrie -- objektiv-mathematisch die Schnittstelle zwischen klassischer Ordnung und moderner Auflösung.

\subsection{Zyklische Evolution: Von Klassik zu Pop}
Eine explorative Erweiterung auf populäre Musik offenbart: Die stilistische Evolution verläuft nicht linear, sondern zyklisch (\hyperref[fig:classicpop]{Abbildung~\ref*{fig:classicpop}}).\footnote{Die Befunde suggerieren eine \enquote{Pendelbewegung} der Musikgeschichte: Nach maximaler harmonischer Komplexität bei Debussy kehrt die populäre Musik des 20. Jahrhunderts zu struktureller Einfachheit zurück. Elton Johns Werk (1970er Jahre) positioniert sich im PCA-Raum zwischen Debussy und Mozart -- er behält impressionistische Klangfarben, vereinfacht aber die harmonische Struktur. Ob diese \enquote{Rückkehr zur Einfachheit} ein universelles Muster musikalischer Evolution nach Perioden maximaler Komplexität darstellt, ist eine offene Forschungsfrage.} Nach maximaler Komplexität bei Debussy kehrt Pop-Musik zu struktureller Einfachheit zurück -- moderne Produktionen nähern sich Mozart-ähnlichen Werten bei Dissonanzrate und harmonischer Dichte.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{OLDvsNEWmusic.jpeg}
    \caption{Zyklische Evolution: Bach $\to$ Mozart $\to$ Chopin $\to$ Debussy $\to$ Pop. Die Trajektorie zeigt nach maximaler Komplexität (Debussy) eine Rückkehr zur Einfachheit.}\label{fig:classicpop}
\end{figure}

\subsection{Reproduzierbarkeit und Limitationen}
\textbf{Stärken:} Vollständige Reproduzierbarkeit (CLI-Tools), balancierte Stichprobe (n=36 pro Komponist), robuste statistische Absicherung (FDR, Tukey-HSD), interpretierbare Merkmale.

\textbf{Limitationen:}\footnote{Detaillierte Erläuterungen: (1) Einzelne Korpus-Einträge sind Sammlungen (z.\,B. ganze Zyklen) oder Duo-Arrangements, die PCA-Cluster beeinflussen können. (2) Entropie-Maße können bei sehr kurzen Stücken stichprobenabhängig sein. (3) Die 51,8\% Restvarianz in höheren Dimensionen (PC4 ff.) modellieren stilistische Details, die in der 3D-Projektion verloren gehen. (4) Performative Aspekte wie Tempo-Rubato und Pedalisierung sind in der symbolischen Analyse prinzipiell nicht erfassbar.} (1) Korpus auf Solo-Klavierwerke aus PDMX beschränkt; (2) PC1--PC3 erfassen 48,2\% der Gesamtvarianz -- stilistische Details in höheren Dimensionen bleiben in 3D-Visualisierung verborgen; (3) Analyse basiert auf Notentexten, nicht Interpretationen (Rubato, Pedalisierung unberücksichtigt); (4) Korpus-Balance reflektiert nicht tatsächliche Produktivität der Komponisten.
...
Für die explorative Analyse wurde eine interaktive Next.js-Weboberfläche entwickelt, die unter \url{https://empirical-music.victorgurbani.com} öffentlich zugänglich ist und Jury sowie interessierten Lesern ermöglicht, die Daten selbstständig zu erforschen. Die Haupt-Dashboard-Seite bietet nahtlosen Zugriff auf dynamische Karussells, welche die Pipeline-Architektur, animierte \enquote{Stylistic Evolution}-GIFs, statistische Signifikanz-Heatmaps und alle Machine-Learning-Visualisierungen bündeln. 

Die spezialisierte \enquote{Subset Clouds}-Seite (\url{https://empirical-music.victorgurbani.com/clouds}) ermöglicht darüber hinaus:
\begin{itemize}
    \item Auswahl beliebig vieler Komponisten-Kombinationen aus dem Feature-Cache, um eigene "Clouds" (PCA-Punktwolken) zu generieren und stilistische Nähe interaktiv in 3D zu überprüfen
...
Das Training des interpretierbaren Random Forests zielte auf eine maximale Reduktion der Komplexität ab, um "White-Box"-Eigenschaften zu bewahren, ohne die Genauigkeit drastisch zu opfern. Die Optimierung erfolgte mittels \texttt{RandomizedSearchCV} (5-Fold Cross-Validation) über folgendes Raster:
\begin{itemize}
...
\subsection{Das Caching-Resolver-Dilemma}
