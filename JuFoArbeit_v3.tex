...
Die Pipeline wurde vollständig in Python~3.10 entwickelt (\texttt{pandas}, \texttt{numpy}, \texttt{scipy}, \texttt{scikit-learn}, \texttt{plotly}, \texttt{music21}). Alle Skripte liegen im Verzeichnis \texttt{src/}; eine visuelle Übersicht der vierstufigen Architektur findet sich in Abbildung~\ref{fig:pipeline} (Anhang~B).
...
\noindent In der 5-Fold Cross-Validation erreichte dieser interpretierbare RF eine Genauigkeit von $\approx 70\,\%$. Die Verwechslungsmatrix (Abbildung~\ref{fig:rf_cm}) zeigt, dass Fehlklassifikationen meist zwischen chronologisch benachbarten Epochen (z.\,B. Chopin und Debussy) auftreten, was die These der fließenden Stilübergänge stützt.
...
Das Ergebnis war überraschend: Der interpretierbare RF schnitt mit $70{,}1\,\%$ Genauigkeit leicht besser ab als das unbeschränkte neuronale Netz ($68{,}7\,\%$; vgl. Abbildung~\ref{fig:nn_comparison}). Wir interpretieren diesen Befund dahingehend, dass die Kombination aus Kollinearitätsfilterung und striktem Pruning als effektive Regularisierung wirkte. Während das neuronale Netz auf dem relativ kleinen Datensatz (144 Werke) zu leichtem Overfitting neigte, zwang die interpretierbare Architektur das Modell dazu, sich auf die robustesten musikalischen Merkmale -- wie den Tonumfang und die harmonische Dichte -- zu konzentrieren (Abbildung~\ref{fig:rf_scatter}).
...
\textbf{Fallstudie 1: Joe Hisaishi.} Die Filmmusik von Hayao Miyazakis Ghibli-Produktionen ist für ihre Synthese aus spätromantischer Harmonik und impressionistischen Klangfarben bekannt. Hisaishis \emph{One Summer's Day} (2001) wurde ohne Vorwissen in den PCA-Raum projiziert -- das Ergebnis (Abbildung~\ref{fig:hisaishi}) bestätigt die Hypothese: Das Stück positioniert sich präzise zwischen Chopin und Debussy, mit leichter Tendenz zu Chopin. Die Methode \emph{sagt korrekt voraus}, wo ein stilistisch hybrider Komponist landen sollte -- ein starker Beleg für die Validität des Merkmalsraums.
...
Abbildung~\ref{fig:pipeline} zeigt den vollständigen Datenfluss von den Rohdaten bis zur Visualisierung.
...
\section{Anhang D: Erweiterte Details zur Machine-Learning-Pipeline}
...
\subsection{D.1 Hyperparameter-Optimierung und Pruning-Statistiken}
...
\subsection{D.2 Beispielhafter Entscheidungsbaum}
...
\subsection{D.3 Komponistenspezifische SHAP-Analysen}
...
\clearpage
\section{Anhang E: Deployment und Web-Infrastruktur}
...
\subsection{E.1 Limitierungen der Standalone-Kompilierung}
...
\subsection{E.2 Abruf dynamischer Cloud-Jobs}
...
\subsection{E.3 Das Caching-Resolver-Dilemma}
