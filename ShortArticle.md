...
- I also ran the highlighter (`src/highlight_pca_piece.py`) on Joe Hisaishi’s *One Summer’s Day (Spirited Away)*—a piece I wavered on tagging as Romantic or Impressionist. The 3D clouds placed it squarely between the Chopin and Debussy clusters, slightly nearer to Chopin, matching how the score alternates lush Romantic progressions with Impressionist shimmer (see `summerdayhighlight.png`).
