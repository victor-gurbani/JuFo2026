...
\begin{itemize}
    \item \textbf{Korpusgröße}: Exakt 144 Werke (36 pro Komponist)
    \item \textbf{Gesamtdauer}: $\sim$73\,000 Viertelnoten
    \item \textbf{ANOVA-Treffer ($\alpha=0.05$)}: 27 Merkmale
    \item \textbf{FDR-robuste Merkmale ($q<0.05$)}: 26 Merkmale
    \item \textbf{PCA-Varianz (PC1--PC3)}: 47,3\%
    \item \textbf{Tukey-Unterschiede Debussy--Mozart}: 16 signifikante Merkmale
    \item \textbf{Tukey-Unterschiede Bach--Mozart}: 0 signifikante Merkmale
\end{itemize}
...
\begin{description}
    ...
    \item[Abweichende PCA-Ergebnisse trotz \texttt{random\_state=42}] Unterschiedliche \texttt{scikit-learn}-Versionen können minimal andere Rotationen erzeugen. \textbf{Lösung}: Exakte Versions-Pinning in \texttt{requirements.txt} (aktuell \texttt{scikit-learn==1.3.0}).
