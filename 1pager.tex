...
Musicology traditionally relies on subjective description to categorize style—classifying Chopin as ``Romantic'' or Debussy as ``Impressionist'' based on qualitative listening. This project aims to replace intuition with rigorous measurement. By building an end-to-end computational pipeline, the objective was to quantify the precise harmonic, melodic, and rhythmic shifts that define the transition from the Common Practice Period (Bach, Mozart) to the late 19th/early 20th century. The goal was to determine if an algorithm could ``hear'' the difference between eras without human bias.

\section*{2. Methodology: The Computational Pipeline}
A license-safe, balanced corpus of \textbf{144 solo piano scores} (36 each by Bach, Mozart, Chopin, and Debussy) was assembled from the PDMX archive. To process this dataset, a Python-based toolkit was engineered utilizing the \texttt{music21} library:
...
\begin{itemize}[leftmargin=*, noitemsep, topsep=0pt]
        ...
        \end{itemize}
        \item \textbf{Statistical Validation:} One-way ANOVA and Tukey HSD post-hoc tests ($p < 0.05$) were applied to identify which features successfully differentiate composers, correcting for False Discovery Rates (FDR) to ensure statistical rigor.
\end{itemize}
...
\begin{wrapfigure}{r}{0.5\textwidth}
        ...
        \href{https://victor-gurbani.github.io/JuFo2026/figures/highlights/ravel_string_quartet_pca_cloud.html}{\includegraphics[width=0.48\textwidth]{ravelhighlight_cropped.png}} 
        \vspace{-10pt}
        \caption{\small \textit{\textbf{Figure 1: PCA Projection of Stylistic Space.} The diamond marker ($\diamond$) represents the unseen test case: Ravel's String Quartet in F. The model correctly projects Ravel beyond the Debussy cluster, quantifying the progression into Post-Impressionism. (\href{https://victor-gurbani.github.io/JuFo2026/figures/highlights/ravel_string_quartet_pca_cloud.html}{\textcolor{blue}{Click to open}}).}}
        \vspace{-10pt}
\end{wrapfigure}
Statistical analysis confirmed that \textbf{29 of the 36 metrics} are significant differentiators. The Principal Component Analysis (PCA) revealed a clear mathematical narrative, wherein the colored clouds represent the training corpus:
...
To test the predictive power of the model, an unseen piece outside the training corpus was introduced: \textbf{Maurice Ravel's \textit{String Quartet in F Major}}. 
...
This project bridges the gap between art and data science, offering a reproducible framework for ``Computational Musicology''. By treating musical scores as data, historical theories can be validated with empirical evidence, proving that the ``feeling'' of a musical era is grounded in quantifiable, mathematical structural changes.
