...
- `python3 src/feature_embedding.py --method pca --output figures/embeddings/pca_3d.html --loadings-csv data/stats/pca_loadings.csv` standardises the feature matrix, applies PCA, and writes an interactive Plotly HTML view (30 numeric dimensions participate after dropping the raw count features listed above).
...
- Passing `--clouds-output figures/embeddings/composer_clouds.html` now generates a companion “four cloud” view that replaces individual markers with semi-transparent Gaussian iso-surfaces per composer. The result foregrounds footprint, overlap, and directional lean in a single glance, complementing the per-piece scatter with a macro-level comparison.
