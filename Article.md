...
- **Raw vs. Corrected Tracking**: We deliberately keep the original α=0.05 `significant` column that first surfaced 27 metrics, and the CLI now logs the 27/11/26 hit counts so readers can see how the added corrections answer the false-positive concerns raised after reviewing those raw results.
...
### Key Findings
- **Multiple-Comparison Context**: Raw α=0.05 lit up 27 metrics, but running 36 ANOVAs in parallel makes the chance of at least one false positive about 1 - 0.95^36 ≈ 84%, so we added Bonferroni (α/36≈0.0014) and Benjamini–Hochberg FDR (q<0.05) controls. Eleven features persist under Bonferroni—led by `pitch_range_semitones`, `dissonance_ratio`, `pitch_class_entropy`, and rhythmic-variance metrics—while 26 remain under FDR, adding expressive contrasts such as `appoggiatura_ratio`, `modal_interchange_ratio`, and `syncopation_ratio` without inflating discoveries.
