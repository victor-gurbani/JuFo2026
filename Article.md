...
- Retained 144 balanced works (36 per composer) spanning 21,344 measures and 71,585 quarter-note durations—about 13.26 listening hours at 90 BPM—after expanding the Debussy resolver to accept typo and initial variants that we manually rechecked score-by-score.
- Parsed structural summaries showing an average of 148.22 measures and 2.10 parts per score, ensuring downstream features reflect comparable texture.
...
- Ran 36 omnibus ANOVAs and 174 Tukey HSD contrasts; raw α=0.05 initially flagged 29 metrics and 62 statistically significant pairwise comparisons, but because running that many tests after our earlier feature engineering inflates false-positive odds we layered in Bonferroni (α/36≈0.0014) and Benjamini–Hochberg FDR (q<0.05) safeguards, leaving 14 and 29 features respectively.
...
- **Multiple-Comparison Control**: Applies both Bonferroni (α/36) and Benjamini–Hochberg FDR corrections to every ANOVA p-value, publishing adjusted columns (`p_value_bonferroni`, `p_value_fdr`) and boolean flags so downstream analyses can choose between family-wise-error protection and higher recall.
- **Raw vs. Corrected Tracking**: We deliberately keep the original α=0.05 `significant` column (29 metrics), and the CLI now logs the 29/14/29 hit counts so readers can see how the added corrections answer the false-positive concerns raised after reviewing those raw results.
...
### Key Findings
- **Multiple-Comparison Context**: Raw α=0.05 lit up 29 metrics, but running 36 ANOVAs in parallel makes the chance of at least one false positive about 1 - 0.95^36 ≈ 84%, so we added Bonferroni (α/36≈0.0014) and Benjamini–Hochberg FDR (q<0.05) controls. Fourteen features persist under Bonferroni—led by `pitch_range_semitones`, `dissonance_ratio`, `pitch_class_entropy`, and rhythmic-variance metrics—while 29 remain under FDR, adding expressive contrasts such as `appoggiatura_ratio`, `modal_interchange_ratio`, and `syncopation_ratio` without inflating discoveries.
...
- The script prints the variance captured by PCs 1–3 (22.3%, 16.1%, 9.8%), clarifying that the axes are weighted blends of metrics—PC1 now balances harmonic density, registral reach, and dissonance ratios rather than being dominated by sheer event totals, PC2 pivots on rhythmic duration spread, and PC3 contrasts pitch-class entropy with downbeat clarity.
...
- **Multi-piece overlays** – `src/highlight_pca_piece.py` now accepts any number of MusicXML inputs in a single run, along with optional `--composer` and `--title` overrides that label each marker explicitly. Highlights render as large dark diamonds outlined in white so every external probe stays legible against the semi-transparent composer clouds, even when several appear at once.
- **External probe** – Curious whether Joe Hisaishi’s *One Summer’s Day (Spirited Away)* leans Romantic or Impressionist, I ran it through `src/highlight_pca_piece.py`. The script appended fresh features, projected the piece into the PCA clouds, and the point landed squarely between the Chopin and Debussy centroids—fractionally nearer to Chopin—mirroring how the score toggles between lush Romantic cadences and Impressionist colour washes (see `summerdayhighlight.png`). This matches my analysis, where parts of the piece evoke Chopin’s melodic language while others embrace Debussy’s textural freedom.
