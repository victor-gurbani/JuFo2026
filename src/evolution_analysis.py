...
def load_and_merge_data():
    ...
    try:
        ...
        merged = harmonic.merge(
            melodic[["mxl_path", "pitch_range_semitones", "avg_melodic_interval"]],
            on="mxl_path",
            suffixes=("", "_mel"),
        )
        merged = merged.merge(
            rhythmic[["mxl_path", "rhythmic_pattern_entropy", "std_note_duration"]],
            on="mxl_path",
            suffixes=("", "_rhy"),
        )
...
def create_evolution_animation(df):
    ...
    def update(frame):
        ...
        fig.suptitle(
            f"Era: {ERA_NAMES[current_step + 1]}", fontsize=16, fontweight="bold"
        )
        if current_step == 3:
