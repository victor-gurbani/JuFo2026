...
def load_and_merge_data():
    ...
    try:
        ...
        merged = harmonic.merge(
            melodic[["mxl_path", "pitch_range_semitones"]], on="mxl_path"
        )
        merged = merged.merge(
            rhythmic[["mxl_path", "rhythmic_pattern_entropy"]], on="mxl_path"
        )
...
def plot_ddd(major_df, minor_df):
    ...
    for i, feature in enumerate(FEATURES_TO_ANALYZE):
        ...
        maj_means = (
            major_df.groupby("composer_label")[feature].mean().reindex(COMPOSER_ORDER)
        )
        min_means = (
            minor_df.groupby("composer_label")[feature].mean().reindex(COMPOSER_ORDER)
        )

        x_vals = range(4)

        ax.plot(
            x_vals, maj_means, marker="o", label="Major", color="#F4A261", linewidth=2.5
        )
        ax.plot(
            x_vals,
            min_means,
